...
const bot_play = document.getElementsByClassName("game_floor_btn");
let t = 0;
let DUMMY_DATA = [1, 2, 3, 4, 5, 6, 7, 8, 9];
let X_arr = []
let O_arr = []
let both_arr = []
//------------------------------------------------------------
//Here Real Code Is Start
const Game = (props) => {
  ...
  const changeplayer = (e) => {
    ...
    X_arr.push(+e.target.getAttribute("number"))
    const index = DUMMY_DATA.indexOf(+e.target.getAttribute("number"));
    if (index > -1) {
      DUMMY_DATA.splice(index, 1);
    }
    t = DUMMY_DATA[getRandomInt(DUMMY_DATA.length - 1)];
    both_arr.push(+e.target.getAttribute("number"))
    props.Check_Arr(X_arr, O_arr, false)
  };
...
//Bot Play From Here
  if (!X) {
    if (DUMMY_DATA.length !== 0 && !props.game_end) {
        let b = 0
        setTimeout(() => {
            if(props.make_des === false) {
                for (const i in text) {
                    let key = 0
                    let counter = 0
                    let ArrToStr = text[i].split(',')
                    let ArrToNum = ArrToStr.map(el => {
                        return Number(el)
                    })
                    for(const x in both_arr) {
                        if(ArrToNum[counter] === both_arr[x]) {
                            counter += 1
                            t = ArrToNum[counter]
                            key = 2
                        } else { 
                ...
                b = bot_play[props.o_game - 1];
            }
            b.nextElementSibling.remove();
            b.nextElementSibling.setAttribute("src", X ? x_player : o_player);

            b.nextElementSibling.removeAttribute("hidden");
            setX((prev) => !prev);
            props.onGetPlayer(b.getAttribute("number"));
            b.setAttribute("disabled", "");
            O_arr.push(+b.getAttribute("number"))

            const index = DUMMY_DATA.indexOf(+b.getAttribute("number"));
            if (index > -1) {
              DUMMY_DATA.splice(index, 1);
            }

            both_arr.push(+b.getAttribute("number"))
        }, 100)
    }
  }
  ...
  useEffect(() => {
    fetch(textfile)
      ...
      .then((textContent) => {
        ...
        for (const i in textContent) {
          textContent[i] = textContent[i].replace("[", "");
          textContent[i] = textContent[i].replace("]", "");
        }
        setText(textContent);
      });
        ...
        >
          <div className="win">
            ...
            <div className="win_btns">
              <button onClick={Start_Game} className="quit_btn">
                Quit
              </button>
